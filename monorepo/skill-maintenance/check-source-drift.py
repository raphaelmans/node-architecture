#!/usr/bin/env python3
"""Validate curated monorepo skill references against canonical source guides."""
...
MONOREPO_DIR = MAINTENANCE_DIR.parent
SKILL_DIR = MONOREPO_DIR / "skill"
MANIFEST_PATH = MAINTENANCE_DIR / "source-map.json"
HASH_ALGORITHM = "sha256(path-null-file-sha256-newline, sources sorted by path)"
EXPECTED_SLICES = {"foundations", "packages", "scaffolding", "turborepo"}
...
def resolve_repo_root(manifest: dict[str, Any], override: str | None) -> Path:
    ...
    if not (root / ".git").exists() or not (root / "monorepo" / "README.md").is_file():
        raise DriftError(
...
def validate_entry(repo_root: Path, name: str, entry: Any) -> str:
    ...
    if not reference.startswith("monorepo/skill/references/"):
        raise DriftError(
            f"Slice {name!r} reference must stay inside monorepo/skill/references"
        )
    ...
    if any(
        not source.startswith("monorepo/")
...
def canonical_sources(repo_root: Path) -> set[str]:
    return {
        ...
        for path in (repo_root / "monorepo").rglob("*.md")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check or refresh monorepo skill source fingerprints."
    )
    parser.add_argument(
        "--repo-root",
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        if command == "refresh":
            ...
            print(f"Refreshed monorepo slice {name}: {actual[name]}")
        ...
        if drifted:
            print("Monorepo skill references require review:", file=sys.stderr)
            for name in drifted:
                print(
        ...
        if command == "check":
            print(
                f"Monorepo skill source map is current "
