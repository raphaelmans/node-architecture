...
- Route handlers and Server Actions are public entrypoints: authenticate independently and invoke the protected server application operation. Coordinate with installed `$server` for capability authorization when available; do not bypass it with direct repository calls.
